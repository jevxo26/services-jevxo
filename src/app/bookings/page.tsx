...
export default function BookingsPage() {
  ...
  if (!isAuthenticated) {
    return (
      <section className="min-h-screen bg-gradient-to-b from-[#EEF2FF]/40 via-white to-white py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex items-center">
        <div className="absolute top-[10%] left-[-10%] w-[350px] h-[350px] bg-[#4F46E5]/4 blur-[110px] rounded-full pointer-events-none" />
        <div className="absolute bottom-[10%] right-[-10%] w-[450px] h-[450px] bg-[#4F46E5]/2 blur-[130px] rounded-full pointer-events-none" />
        ...
        <div className="mx-auto max-w-2xl w-full text-center relative z-10 space-y-6">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 120 }} className="inline-flex p-4.5 bg-[#EEF2FF] text-[#4F46E5] rounded-3xl border border-[#4F46E5]/15 shadow-sm shadow-[#4F46E5]/5 mb-2">
          ...
          <motion.h1 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-xl md:text-2xl lg:text-3xl font-medium text-slate-900 tracking-tight leading-tight">
            Manage Your <span className="text-[#4F46E5]">Schedules</span>
          </motion.h1>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-left">
            <motion.div variants={cardFadeUp} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:border-[#4F46E5]/10 transition-colors">
              <div className="w-8 h-8 bg-[#EEF2FF] text-[#4F46E5] rounded-xl flex items-center justify-center mb-3"><Clock className="w-4 h-4" /></div>
            ...
            <motion.div variants={cardFadeUp} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:border-[#4F46E5]/10 transition-colors col-span-2 sm:col-span-1">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/login" className="w-full sm:w-auto">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#4F46E5] hover:bg-[#4338CA] text-white font-extrabold py-3.5 px-8 rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer text-xs uppercase tracking-wider">
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#EEF2FF]/20 via-white to-white px-4 py-8 md:py-12 lg:px-8 relative overflow-hidden">
      <div className="absolute top-[5%] left-[-10%] w-[350px] h-[350px] bg-[#4F46E5]/3 blur-[120px] rounded-full pointer-events-none" />
      <div className="absolute bottom-[20%] right-[-10%] w-[450px] h-[450px] bg-[#4F46E5]/2 blur-[150px] rounded-full pointer-events-none" />
      <div className="absolute inset-0 bg-[url('/bg-icons-design.png')] bg-repeat opacity-10 pointer-events-none z-0" style={{ backgroundSize: 'auto' }} />
      ...
      <div className="mx-auto max-w-5xl relative z-10">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6 mb-8">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-[#EEF2FF] text-[#4F46E5] rounded-2xl border border-[#4F46E5]/15 shadow-sm shadow-[#4F46E5]/2">
        ...
        <motion.div layout className="space-y-4">
          {isBookingsLoading ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="py-20 text-center bg-white rounded-3xl border border-slate-100 shadow-sm space-y-4">
              <div className="w-12 h-12 rounded-full bg-[#EEF2FF] text-[#4F46E5] flex items-center justify-center mx-auto border border-[#4F46E5]/10"><FileText className="w-6 h-6" /></div>
              ...
              {!search && (<Link href="/services" className="inline-block pt-1"><button className="bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs font-extrabold py-3 px-6 rounded-xl transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-md hover:shadow-[0_8px_20px_rgba(79, 70, 229,0.2)] uppercase tracking-wider">Book a Service</button></Link>)}
            </motion.div>
          ) : (
                  ...
                  <motion.div key={booking.id} layout initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring", stiffness: 100, damping: 15 }} whileHover={{ y: -3, boxShadow: "0 12px 30px -5px rgba(0,0,0,0.03)" }} className={`flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-2xl border-l-4 bg-white p-5 border border-slate-100/90 transition-all ${cfg.borderColor} ${cfg.glowColor}`}>
                    ...
                    <div className="flex flex-row md:flex-col md:items-end justify-between items-center gap-3 pt-3.5 md:pt-0 border-t border-slate-50 md:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => setSelectedBooking(booking)} className="rounded-xl border border-slate-200 hover:border-[#4F46E5] hover:text-[#4F46E5] bg-slate-50/50 hover:bg-[#EEF2FF] px-4 py-2.5 text-xs font-extrabold text-slate-600 transition-all hover:scale-105 active:scale-95 cursor-pointer flex items-center gap-1">
