  ...
  return (
    <div className="relative bg-transparent flex-1 flex flex-col">
      ...
      <section ref={heroRef} className="relative pt-12 pb-10 md:pt-16 md:pb-12">
        <motion.div style={{ y: glowY }} className="pointer-events-none absolute -top-32 right-0 w-[500px] h-[500px] bg-[#4F46E5]/5 blur-[120px] rounded-full" />
        <motion.div style={{ y: glowY2 }} className="pointer-events-none absolute -bottom-16 left-1/4 w-[300px] h-[300px] bg-[#4F46E5]/4 blur-[100px] rounded-full" />
        <div className="pointer-events-none absolute top-0 right-0 w-64 h-64 border-l border-b border-[#4F46E5]/6 rounded-bl-full" />
        <div className="max-w-3xl mx-auto px-4 md:px-6 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="inline-flex items-center gap-2 text-[10px] font-extrabold text-[#4F46E5] uppercase tracking-[.12em] bg-[#FFF4EE] px-3.5 py-1.5 rounded-full border border-[#4F46E5]/20 mb-5">
            <span className="w-1.5 h-1.5 rounded-full bg-[#4F46E5] animate-pulse" />Support Center
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.08 }} className="text-2xl md:text-3xl lg:text-4xl font-medium text-slate-900 tracking-[-0.03em] leading-[1.12] mb-4">
            How can we <span className="relative inline-block text-[#4F46E5]">help you today?<span className="absolute bottom-1 left-0 w-full h-[3px] bg-[#4F46E5]/15 rounded-full" /></span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.24 }} className="flex flex-wrap justify-center gap-2">
            {TRUST_BARS.map(({ icon: Icon, text }) => (
              <span key={text} className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-50 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-full">
                <Icon className="w-3 h-3 text-[#4F46E5]" />{text}
              </span>
            ))}
          </motion.div>
        </div>
          ...
          <RevealSection>
            <div className="grid md:grid-cols-3 gap-3">
              {CONTACT_CHANNELS.map(({ icon: Icon, label, primary, href, secondary, badge }) => (
                <motion.a key={label} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noopener noreferrer" : undefined} whileHover={{ y: -2 }} transition={{ duration: 0.2 }} className="group relative bg-white border border-slate-100 rounded-2xl p-5 flex items-start gap-4 hover:border-[#4F46E5]/25 hover:shadow-[0_4px_24px_rgba(79, 70, 229,0.08)] transition-all duration-300 overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#4F46E5]/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                  <span className="relative p-2.5 rounded-xl bg-[#FFF4EE] text-[#4F46E5] flex-shrink-0 border border-[#4F46E5]/10"><Icon className="w-4.5 h-4.5" /></span>
                  <div className="relative min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-black text-[9px] uppercase tracking-[.12em] text-slate-400">{label}</h3>
                      <span className="text-[9px] font-bold text-[#4F46E5] bg-[#FFF4EE] border border-[#4F46E5]/15 px-2 py-0.5 rounded-full whitespace-nowrap">{badge}</span>
                    </div>
                    <p className="text-[13px] font-black text-slate-800 group-hover:text-[#4F46E5] transition-colors truncate">{primary}</p>
            ...
            <RevealSection className="lg:col-span-5 flex flex-col gap-4" delay={0.1}>
              <div className="relative rounded-2xl overflow-hidden h-[210px] border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                ...
                <div className="relative z-10 p-5 h-full flex flex-col justify-end">
                  <span className="inline-block text-[9px] font-black tracking-[.1em] text-[#4F46E5] bg-white/95 px-2.5 py-1 rounded-full uppercase mb-2 w-fit">Headquarters</span>
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
                <div className="flex items-center gap-2 mb-4"><div className="p-1.5 rounded-lg bg-[#FFF4EE]"><Clock className="w-3.5 h-3.5 text-[#4F46E5]" /></div><h4 className="font-black text-[10px] text-slate-800 uppercase tracking-[.1em]">Office Hours</h4></div>
              ...
              <div className="bg-[#FFF4EE] border border-[#4F46E5]/10 rounded-2xl p-5">
                ...
                <div className="flex gap-2">
                  {SOCIAL_LINKS.map(({ icon: Icon, href, label }) => (
                    <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="w-8 h-8 rounded-full bg-white border border-[#4F46E5]/15 text-[#4F46E5] flex items-center justify-center hover:bg-[#4F46E5] hover:text-white hover:border-[#4F46E5] transition-all duration-200 hover:scale-110">
      ...
      <section className="pt-10 pb-4 md:py-14">
        <div className="max-w-3xl mx-auto px-4 md:px-6">
          <RevealSection className="text-center mb-8">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold text-[#4F46E5] uppercase tracking-[.12em] bg-[#FFF4EE] px-3.5 py-1.5 rounded-full border border-[#4F46E5]/20 mb-4"><HelpCircle className="w-3 h-3" />General Help</span>
          ...
          <RevealSection delay={0.05}>
            <div className="space-y-2">
              {FAQS.map((faq, i) => {
                ...
                return (
                  <div key={i} className={`bg-white border rounded-xl overflow-hidden transition-all duration-200 ${isOpen ? "border-[#4F46E5]/20 shadow-[0_2px_12px_rgba(79, 70, 229,0.06)]" : "border-slate-100"}`}>
                    <button type="button" onClick={() => setActiveFaq(isOpen ? null : i)} className="w-full flex items-center justify-between gap-3 p-4 text-left outline-none">
                      <span className={`text-[12px] font-bold transition-colors ${isOpen ? "text-[#4F46E5]" : "text-slate-800"}`}>{faq.question}</span>
                      <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ${isOpen ? "rotate-180 text-[#4F46E5]" : "text-slate-300"}`} />
                    </button>
                    <AnimatePresence>
                      {isOpen && (
