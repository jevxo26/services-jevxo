...
export default function OperationalInsights({
  ...
  return (
    <div className="lg:col-span-1 bg-white rounded-3xl border border-slate-100 hover:border-[#4F46E5]/15 hover:shadow-lg hover:shadow-[#4F46E5]/5 transition-all duration-300 p-6 flex flex-col justify-between">
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-slate-50/60 p-3 rounded-2xl border border-slate-100/50 flex flex-col">
            ...
            <span className="text-sm font-black text-[#4F46E5] mt-1">{totalVendorsRegistered} Registered</span>
          </div>
        </div>
              ...
              topVendors.map((vendor: any, idx: number) => (
                ...
                >
                  {vendor.avatar ? (
                    ...
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#4F46E5]/10 to-[#FFBAB4]/10 flex items-center justify-center text-xs font-bold text-[#4F46E5]">
                      {vendor.company_name?.substring(0, 2).toUpperCase() || "VD"}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-extrabold text-slate-800 truncate group-hover/v:text-[#4F46E5] transition-colors">
                      {vendor.company_name}
                    </p>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-[#4F46E5] font-bold mt-0.5">৳{vendor.min_starting_price || 400}+</p>
                  </div>
                </div>
              ))
            )}
      ...
      <div className="mt-5 pt-3 border-t border-slate-100">
        <Link
          ...
          className="w-full py-2.5 bg-slate-50 hover:bg-[#4F46E5] hover:text-white rounded-2xl border border-slate-100 hover:border-[#4F46E5] transition-all text-xs font-bold text-slate-600 flex items-center justify-center gap-1 shadow-2xs"
        >
