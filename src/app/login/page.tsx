        ...
        <div className="flex-1 flex flex-col justify-center items-center px-4 sm:px-8 py-6 relative z-10">
          ...
          {!isOtpSent ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input type="tel" name="phone" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="01XXX-XXXXXX" className="w-full pl-[88px] pr-5 py-4 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-[#1E4E8C] focus:ring-4 focus:ring-[#1E4E8C]/10 focus:outline-none transition-all text-xs font-semibold text-slate-800 placeholder-slate-400" required />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/help" className="text-xs text-[#1E4E8C] hover:underline font-bold">Need help?</Link>
                </div>
            ...
            <div className="w-full max-w-[400px] mx-auto">
              <div className="mb-10">
                <div className="w-16 h-16 bg-gradient-to-br from-[#1E4E8C] to-[#FF8142] rounded-2xl flex items-center justify-center text-white mx-auto mb-6 shadow-xl shadow-[#1E4E8C]/30">
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-6">
                <div className="flex justify-center gap-3">
                  {otp.map((digit, idx) => (
                    <input key={idx} type="text" inputMode="numeric" maxLength={1} value={digit} ref={(el) => { otpInputsRef.current[idx] = el }} onChange={(e) => handleOtpChange(e.target.value, idx)} onKeyDown={(e) => handleOtpKeyDown(e.key, idx)} className={`w-14 h-16 rounded-2xl border-2 text-center text-2xl font-black focus:outline-none transition-all shadow-sm ${digit ? "border-[#1E4E8C] bg-[#EEF2FF] text-[#1E4E8C]" : "border-slate-100 bg-slate-50 text-slate-800 focus:border-[#1E4E8C] focus:bg-white focus:ring-4 focus:ring-[#1E4E8C]/10"}`} required />
                  ))}
                </div>
                ...
                <div className="text-center">
                  {timeLeft > 0 ? (
                    <div className="inline-flex items-center gap-2 text-slate-500 text-xs font-semibold">
                      <Clock size={14} className="text-[#1E4E8C]/80" />
                      Resend code in <span className="text-[#1E4E8C] font-black tabular-nums">{Math.floor(timeLeft / 60)}:{timeLeft % 60 < 10 ? "0" + (timeLeft % 60) : timeLeft % 60}</span>
                    </div>
                  ) : (
                    <button type="button" onClick={handleResendOtp} className="text-[#1E4E8C] hover:underline font-black text-xs flex items-center gap-1 mx-auto border-none bg-transparent cursor-pointer">
                      Resend Code <ChevronRight size={14} />
                    </button>
                  )}
                </div>

                <button type="submit" disabled={isVerifying || otp.join("").length < 4} className="w-full bg-[#1E4E8C] hover:bg-[#123C73] disabled:opacity-50 text-white text-[11px] font-extrabold tracking-wide py-4 rounded-2xl shadow-xl shadow-[#1E4E8C]/25 active:scale-[0.99] transition-all flex items-center justify-center gap-2 focus:outline-none border-none cursor-pointer">
